...
import {formatAmount, updateLatestIndicators,multipliedBy,calculateMA, dividedBy,formatTimestamp, setStyle} from './utils/util';
...
// console.log("十四师-------")
    ...
    this.candleSeries = this.chart.addSeries(CandlestickSeries,{
      ...
      this.candleSeries.attachPrimitive(new OverlayPriceScale({side:"right"}));
    ...
    this.chart.timeScale().subscribeVisibleLogicalRangeChange((logicalRange) => {
      ...
      if (logicalRange && logicalRange.from < 30  && !this._loadingMore && this._isInit) {
  ...
  public setData(data:any[],priceDecimal?:number): void {
   ...
   let lastObj = updateLatestIndicators(candleData, this._ma5Val,  this._ma10Val )
    this.setVOL(lastObj)
// this.setPriceData()
  ...
  public update(bar: KLineBar): void {
   ...
   this._ma5Val = calculateMA( this._volumeList, 5);
    this._ma10Val = calculateMA( this._volumeList, 10);
   let lastObj = updateLatestIndicators(this._data, this._ma5Val, this._ma10Val)
  ...
  this.chart.subscribeClick((param) => {
    ...
    if (
      ...
      let  rangeRatioColor  =Number(middleValue) <  0?this._downColor:this._upColor
     let amplitude = dataItem.high
  ? (Math.abs(Number(dataItem.high) - Number(dataItem.low)) / Number(dataItem.open) * 100).toFixed(2) + '%'
  : "";

      let symbolChar =    Number(middleValue) == 0 ? "":( Number(middleValue)<0?"":"+")
      ...
      <div style="display: flex;justify-content: space-between;margin-bottom: 5px;">
       ...
       <div style="display: flex;justify-content: space-between;margin-bottom: 5px;">
        ...
        <div style="display: flex;justify-content: space-between;margin-bottom: 5px;">
         <span style="color:rgba(13, 12, 34, 0.5)">${Il8n[this._language].amplitude}:</span>
        <span>${amplitude}</span>
      </div>
      </div>`
  ...
  private setPriceData() {
    ...
    this.chart.timeScale().subscribeVisibleLogicalRangeChange(this.renderCustomPriceScale);
    this.candleSeries.priceScale().subscribePriceScaleChanged(this.renderCustomPriceScale);
